<?php
/**
 * Generic Single Post & Custom Post Type Fallback Template
 *
 * @package Wheelhouse
 */
...
<main class="main-content single-project-main">
	...
	<section class="about-hero-section project-single-hero-section">
		<div class="hero-card-wrapper about-hero-wrapper">
			<div class="about-hero-card project-single-hero-card" style="background-image: linear-gradient(180deg, rgba(21, 9, 36, 0.75) 0%, rgba(35, 14, 58, 0.88) 100%), url('<?php echo esc_url( ! empty( $project['image'] ) ? $project['image'] : get_the_post_thumbnail_url( get_the_ID(), 'full' ) ); ?>') !important;">
				...
				<div class="project-single-top-bar flex-between align-center">
					...
					<div class="projects-tag flex-center hero-tag">
						<span class="tag-dot"></span>
						<span class="text-sm"><?php echo esc_html( ! empty( $project['category'] ) ? $project['category'] : 'PROJECT' ); ?></span>
					</div>
				</div>
			...
			<div class="project-single-content-card">
				<div class="project-modal-rich-text single-project-rich-text">
					...
					if ( ! empty( $project['description'] ) ) {
						echo wp_kses_post( $project['description'] );
					} else {
						the_content();
					}
					?>
				...
				<div class="project-single-gallery-grid">
					<?php foreach ( $project['gallery'] as $g_url ) : ?>
						<div class="single-gallery-item">
							<img src="<?php echo esc_url( $g_url ); ?>" alt="<?php echo esc_attr( get_the_title() ); ?> Photo" class="single-gallery-img" loading="lazy">
						</div>
					<?php endforeach; ?>
				</div>
			</div>
